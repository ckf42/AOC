...
packageList = tuple(tuple(l.splitlines()) for l in packageGroup)
...
def isInOrder(a, b):
    if isinstance(a, ast.Constant) and isinstance(b, ast.Constant):
        ...
        return isInOrder(a, ast.parse('[' + ast.unparse(b) + ']', mode='eval').body)
    else:
        la = len(a.elts)
        lb = len(b.elts)
...
# part 1
counter = 0
for idx in range(len(packageList)):
    if isInOrder(ast.parse(packageList[idx][0], mode='eval').body,
                 ast.parse(packageList[idx][1], mode='eval').body) == -1:
        counter += 1 + idx
print(counter)

# part 2
packages = tuple((pl, False) for gp in packageList for pl in gp) \
        + tuple(zip(('[[2]]', '[[6]]'), (True, True)))
packageStream = list((ast.parse(p, mode='eval').body, v) for (p, v) in packages)
